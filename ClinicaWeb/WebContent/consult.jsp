...
<article class="module width_full">
			...
			<% 
				...
				out.println("<h3>Obs: Apenas uma consulta pode ser marcada por vez! </h3>");
				out.println("</fieldset>");
			
				for (Consulta c: consultas){	
					if(c.getIdDisponibilidade() != 1){
					out.println("<fieldset>");	
					out.println("<table>");
					out.println("<form action='consult-final.jsp' method='post'>");
					out.println("<tr>");
					out.println("<td>");
					out.println("<div class='module_content'>");
					out.println("<label>Marcar Consulta:</label>");
					...
					session.setAttribute("disponibilidade",c.getIdDisponibilidade());
					session.setAttribute("data", c.getData());
					session.setAttribute("inicio", c.getInicio());
					session.setAttribute("fim", c.getFim());
					session.setAttribute("observacoes", c.getObservacoes());
					session.setAttribute("prescricao", c.getPrescricoes());
					session.setAttribute("recomendacoes", c.getRecomendacoes());
					out.println("</div>");
					out.println("</td>");
					out.println("</form>");
					out.println("</table>");
